...
contract FullRedemptionAndLPExitTest is ProtocolTestUtils {
    ...
    uint256 constant LOWER_PRICE = 70 * 1e18;   // $70.00 per asset (30% decrease)
     ...
     */
    function testFullRedemptionHigherPriceAndLPExit() public {
    ...
    /**
     * @notice Main integration test: Full user redemption at lower price + LP exit
     * @dev This tests the complete flow that could potentially cause underflows
     */
    function testFullRedemptionLowerPriceAndLPExit() public {
        ...
        // User redeems ALL their assets at the lower price
        vm.startPrank(user1);
        assetToken.approve(address(assetPool), userAssetBalance);
        assetPool.redemptionRequest(userAssetBalance);
        vm.stopPrank();
        ...
        // Complete cycle to process the redemption
        completeCycleWithPriceChange(LOWER_PRICE);
        ...
        // Calculate expected redemption amount at lower price
        uint256 expectedRedemptionReserve = getExpectedReserveAmount(userAssetBalance, LOWER_PRICE);
        uint256 expectedTotalRedemption = expectedRedemptionReserve + collateralAmount;

        // User should receive less than initial deposit due to price decrease
        assertLt(redemptionPayout, totalUserDeposit, "User should receive less due to price decrease");
        assertApproxEqRel(redemptionPayout, expectedTotalRedemption, 0.03e18, "Redemption payout should be approximately correct");
