...
contract SimpleToken is ERC20, Ownable {
    ...
    /**
     * @dev Constructor that sets the token name, symbol, and non-owner mint limit
     * @param name_ The name of the token
     * @param symbol_ The symbol of the token
     * @param decimals_ The number of decimals the token uses
     * @param mintLimit_ The maximum amount of tokens a non-owner can mint (in smallest units)
     */
    constructor(
        string memory name_,
    ...
    function mint(uint256 amount) external {
        if (owner() == _msgSender()) {
            ...
            // Non-owners have a minting limit
